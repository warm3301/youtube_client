...
def generate_symb()->str:
    return choice(list(ascii_uppercase) + list(ascii_lo)+map(str, range(0,10)))
#TODO update
def generate_filename(ext:str,direcotry:str=None,base:str=None)->str:
    """param base: without directory"""
    res = base if base else generate_symb()
    while True:
        filename = f"{res}.{ext}"
        if direcotry and not os.path.isfile(os.path.join(direcotry,filename)):
            return filename
...
def self_lru_cache(func: Callable[..., GenericType]) -> GenericType:
    """ mypy compatible annotation wrapper for lru_cache"""
    return functools.lru_cache()(func)  # type: ignore
    
def cached(func):
    def inner(self, *args, **kwargs):
        res = self._cache.get(func.__name__)
        if res:
            return res
