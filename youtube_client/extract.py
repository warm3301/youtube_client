...
from .exceptions import HTMLParseError, LiveStreamError, RegexMatchError
from .helpers import regex_search
from .parser import parse_for_object, parse_for_all_objects
import urllib
...
def time_from_url(url:str)->int:
    """extract parametr for example t=90 and return int 90"""
    parsed = urllib.parse.urlparse(url)
    res = parse_qs(parsed.query).get('t')#TODO 1h32m5s
    if res and len(res) > 0:
        res = res[0]
        if res[-1].isdigit():
            return int(res)
        elif res.endswith("s"):
            return int(res.replace('s'))
        elif res.endswith("m"):
            return int(float(res.replace("m")) * 60)
        elif res.endswith("h"):
            return int(float(res.replace("h")) * 3600)
        else:
            raise NotImplementedError("extract.time_from_url")
    return None
...
def playlist_id(url: str) -> str:
    ...
    - :samp: `https://youtu.be/{id}?si={plid}`
    ...
    if parsed.hostname == 'youtu.be':
        return parse_qs(parsed.query)['si'][0]
    return parse_qs(parsed.query)['list'][0]

def playlist_video_id(url: str) -> str:
    """Extract the ``playlist_id`` from a YouTube url.
    ...
    for pattern in config_patterns:
        ...
        try:
            ...
            if res["playabilityStatus"]["status"] == "ERROR":
                raise Exception(res["playabilityStatus"]["status"] + "; "+res["playabilityStatus"]["reason"])
            return res
...
def append_foreach(raw,key,value):
    for x in raw:
        x[key] = value
    return raw
def apply_descrambler(stream_data: Dict) -> None:
    ...
    **Example**:
    ...
    if 'formats' in stream_data.keys():
        formats.extend(append_foreach(stream_data['formats'],"is_adaptive",False))
    if 'adaptiveFormats' in stream_data.keys():
        formats.extend(append_foreach(stream_data['adaptiveFormats'],"is_adaptive",True))
...
def initial_data(watch_html: str) -> str:
    ...
    for pattern in patterns:
        try:
            ...
            try:
                message_container = res["contents"]["twoColumnWatchNextResults"]["results"]["results"]["contents"][0]["itemSectionRenderer"]["contents"][0]
                if "backgroundPromoRenderer" in message_container:
                    raise Exception(" ".join([x["text"] for x in message_container["backgroundPromoRenderer"]["title"]["runs"]]))
            except KeyError: pass
            try:
                message_container = res["alerts"][0]["alertRenderer"]
                if message_container["type"] == "ERROR":
                    raise Exception(" ".join([x["text"] for x in message_container["text"]["runs"]]))
            except KeyError:pass
            return res
        except HTMLParseError:
            pass
